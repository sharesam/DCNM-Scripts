...
wb = load_workbook(
    '/Users/krishna/python-projects/My_venvs/venv1-enterprise/dcnm-python-Testing/Fabric-Switch-Onboarding/Spine-Onboarding/Spine-Details.xlsx')
...
for i in range(2, len(spine_onboarding['A']) + 1):
    ...
    requests.post(url_poap, headers=headers_token, verify=False,
                  data=json.dumps(spine_details))  #### Poaps the Spine Switch
    spine_role = [{"serialNumber": spine_onboarding[f'A{i}'].value, "role": "Spine"}]
    requests.post(url_role, headers=headers_token, verify=False,
                  data=json.dumps(spine_role))  #### Assigns Spine Role to the switch
